...
usage() {
  cat <<'EOF'
Usage: ./build_perception_arm.sh [options]
...
apply_module_build_flags() {
  update_module_build_flags "${ROOT_DIR}/modules/perception/lidar/lidar_ground_segmentation/scripts/build.arm64.sh"
  update_module_build_flags "${ROOT_DIR}/modules/perception/lidar/lidar_cluster_detect/scripts/build.arm64.sh"
  update_module_build_flags "${ROOT_DIR}/modules/perception/lidar/multi_lidar_splicing/scripts/build.arm64.sh"
  update_module_build_flags "${ROOT_DIR}/modules/perception/fusion/motion_manager/scripts/build.arm64.sh"
}
...
generate_vehicle_proto() {
  ...
  local protoc_bin=""
  if [ -x /usr/local/legion/third_party/arm64/bin/protoc ]; then
    protoc_bin="/usr/local/legion/third_party/arm64/bin/protoc"
    export LD_LIBRARY_PATH="/usr/local/legion/third_party/arm64/lib/protobuf:${LD_LIBRARY_PATH:-}"
