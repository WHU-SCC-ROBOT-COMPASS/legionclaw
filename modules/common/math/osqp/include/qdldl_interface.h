...
// This used only in non embedded or embedded 2 version
#if EMBEDDED != 1
  c_int (*update_matrices)(
      struct qdldl* self, const csc* P, const csc* A,
      const OSQPSettings* settings); ///< Update solver matrices
  c_int (*update_rho_vec)(struct qdldl* self, const c_float* rho_vec,
                          const c_int m); ///< Update solver matrices
#endif
  ...
  csc* KKT; ///< Permuted KKT matrix in sparse form (used to update P and A
            ///< matrices)
 ...
 * @param	rho_vec Algorithm parameter. If polish, then rho_vec =
 * OSQP_NULL.
...
c_int update_linsys_solver_rho_vec_qdldl(qdldl_solver* s,
                                         const c_float* rho_vec, const c_int m);
