...
GLOBAL void
AMD_2(Int n,     /* A is n-by-n, where n > 0 */
      ...
      Int Head[], Int Elen[], /* the size columns of L for each supernode */
      Int Degree[], Int W[],
   ...
   * (excluding the diagonal) perform an approximate minimum
   (UMFPACK/MA38-style)
   ...
   * Library).
   *
   * The approximate degree algorithm implemented here is the symmetric analog
   of
   ...
   *      descriptions below and integer is and int or SuiteSparse_long
   depending
   ...
   * The UMFPACK/MA38 approximate degree update algorithm, the unsymmetric
   analog
   * which forms the basis of AMD, was developed while Tim Davis was supported
   by
   * CERFACS (Toulouse, France) in a post-doctoral position.  This C version,
   and
   ...
   * ----------------------------------------------------------------------------
   *
   * Nv:  An integer array of size n.  During execution, ABS (Nv [i]) is equal
   to *	the number of rows that are represented by the principal supervariable
   ...
   *	matrix, not the permuted matrix.
   *
   * Elen:  An integer array of size n.  See the description of Iw above.  At
   the *	start of execution, Elen [i] is set to zero for all rows i.
   During *	execution, Elen [i] is the number of elements in the list for
  ...
  Int deg, degme, dext, lemax, e, elenme, eln, i, ilast, inext, j, jlast, jnext,
      k, knt1, knt2, knt3, lenj, ln, me, mindeg, nel, nleft, nvi, nvj, nvpiv,
      slenme, wbig, we, wflg, wnvi, ok, ndense, ncmpa, dense, aggressive;
  ...
  */
   ...
   * Pointer) is an index into Iw, and all indices into Iw use variables
   * starting with "p."  The only exception to this rule is the iwlen input
   * argument.
   *
...
#ifndef NDEBUG
  AMD_DEBUG1(("\n======Nel " ID " initial\n", nel));
  AMD_dump(n, Pe, Iw, Len, iwlen, pfree, Nv, Next, Last, Head, Elen, Degree, W,
           -1);
#endif
  ...
  while (nel < n) {
    ...
    AMD_DEBUG1(("\n======Nel " ID "\n", nel));
    if (AMD_debug >= 2) {
      AMD_dump(n, Pe, Iw, Len, iwlen, pfree, Nv, Next, Last, Head, Elen, Degree,
               W, nel);
    }
#endif
  ...
  */
  ...
  AMD_postorder(n, Pe, Nv, Elen, W, /* output order */
