...
void free_linsys_solver_pardiso(pardiso_solver* s) {
  if (s) {
    ...
    pardiso(s->pt, &(s->maxfct), &(s->mnum), &(s->mtype), &(s->phase), &(s->n),
            &(s->fdum), s->KKT->p, s->KKT->i, &(s->idum), &(s->nrhs), s->iparm,
...
pardiso_solver* init_linsys_solver_pardiso(const csc* P, const csc* A,
  ...
  if (polish) { // Called from polish()
    ...
    s->KKT = form_KKT(P, A, 1, sigma, s->bp, OSQP_NULL, OSQP_NULL, OSQP_NULL,
                      OSQP_NULL, OSQP_NULL);
    ...
    s->KKT = form_KKT(P, A, 1, sigma, s->bp, s->PtoKKT, s->AtoKKT,
...
// Set MKL interface layer (Long integers if activated)
  ...
  s->iparm[7] = 0;  // Number of iterative refinement steps (auto, performs them
                    // only if perturbed pivots are obtained)
  ...
  pardiso(s->pt, &(s->maxfct), &(s->mnum), &(s->mtype), &(s->phase), &(s->n),
          s->KKT->x, s->KKT_p, s->KKT_i, &(s->idum), &(s->nrhs), s->iparm,
          &(s->msglvl), &(s->fdum), &(s->fdum), &(s->error));
  if (s->error != 0) {
#ifdef PRINTING
...
c_int update_linsys_solver_matrices_pardiso(pardiso_solver* s, const csc* P,
                                            const csc* A,
                                            const OSQPSettings* settings) {
