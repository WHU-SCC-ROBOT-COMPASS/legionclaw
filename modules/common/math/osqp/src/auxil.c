...
c_float compute_rho_estimate(OSQPWorkspace* work) {
  ...
  // Return rho estimate
  rho_estimate =
      work->settings->rho * c_sqrt(pri_res / (dua_res + 1e-10)); // (prevent
                                                                 // 0
                                                                 // division)
...
c_int update_rho_vec(OSQPWorkspace* work) {
  ...
  if (constr_type_changed == 1) {
    exitflag = work->linsys_solver->update_rho_vec(
        work->linsys_solver, work->rho_vec, work->data->m);
  }
...
static void compute_rhs(OSQPWorkspace* work) {
  ...
  for (i = 0; i < work->data->n; i++) {
    ...
    work->xz_tilde[i] =
        work->settings->sigma * work->x_prev[i] - work->data->q[i];
  }

  for (i = 0; i < work->data->m; i++) {
    // Cycle over dual variable in the first step (nu)
    work->xz_tilde[i + work->data->n] =
        work->z_prev[i] - work->rho_inv_vec[i] * work->y[i];
  }
}
...
static void update_z_tilde(OSQPWorkspace* work) {
  ...
  for (i = 0; i < work->data->m; i++) {
    work->xz_tilde[i + work->data->n] =
        work->z_prev[i] +
        work->rho_inv_vec[i] * (work->xz_tilde[i + work->data->n] - work->y[i]);
  }
}
...
c_float compute_obj_val(OSQPWorkspace* work, c_float* x) {
  ...
  obj_val =
      quad_form(work->data->P, x) + vec_prod(work->data->q, x, work->data->n);
...
c_float compute_pri_res(OSQPWorkspace* work, c_float* x, c_float* z) {
  ...
  if (work->settings->scaling && !work->settings->scaled_termination) {
    return vec_scaled_norm_inf(work->scaling->Einv, work->z_prev,
                               work->data->m);
  }
...
c_float compute_pri_tol(OSQPWorkspace* work, c_float eps_abs, c_float eps_rel) {
  ...
  if (work->settings->scaling && !work->settings->scaled_termination) {
    ...
    // ||Einv * A * x||
    temp_rel_eps =
        vec_scaled_norm_inf(work->scaling->Einv, work->Ax, work->data->m);
...
c_float compute_dua_tol(OSQPWorkspace* work, c_float eps_abs, c_float eps_rel) {
  ...
  if (work->settings->scaling && !work->settings->scaled_termination) {
    // || Dinv q||
    max_rel_eps =
        vec_scaled_norm_inf(work->scaling->Dinv, work->data->q, work->data->n);

    // || Dinv A' y ||
    temp_rel_eps =
        vec_scaled_norm_inf(work->scaling->Dinv, work->Aty, work->data->n);
    max_rel_eps = c_max(max_rel_eps, temp_rel_eps);

    // || Dinv P x||
    temp_rel_eps =
        vec_scaled_norm_inf(work->scaling->Dinv, work->Px, work->data->n);
    max_rel_eps = c_max(max_rel_eps, temp_rel_eps);
...
c_int is_primal_infeasible(OSQPWorkspace* work, c_float eps_prim_inf) {
  ...
  if (work->settings->scaling &&
      !work->settings->scaled_termination) { // Unscale
                                             // if
                                             // necessary
    ...
    if (ineq_lhs < -eps_prim_inf * norm_delta_y) {
      ...
      if (work->settings->scaling &&
          !work->settings->scaled_termination) { // Unscale
                                                 // if
                                                 // necessary
        vec_ew_prod(work->scaling->Dinv, work->Atdelta_y, work->Atdelta_y,
                    work->data->n);
      }
...
c_int is_dual_infeasible(OSQPWorkspace* work, c_float eps_dual_inf) {
  ...
  if (work->settings->scaling &&
      !work->settings->scaled_termination) { // Unscale
                                             ...
                                             // necessary
    norm_delta_x =
        vec_scaled_norm_inf(work->scaling->D, work->delta_x, work->data->n);
  ...
  if (norm_delta_x > eps_dual_inf) {
    ...
    if (vec_prod(work->data->q, work->delta_x, work->data->n) <
      ...
      // Scale if necessary
      if (work->settings->scaling && !work->settings->scaled_termination) {
        vec_ew_prod(work->scaling->Dinv, work->Pdelta_x, work->Pdelta_x,
                    work->data->n);
      }
      ...
      if (vec_norm_inf(work->Pdelta_x, work->data->n) <
        ...
        // Scale if necessary
        if (work->settings->scaling && !work->settings->scaled_termination) {
          vec_ew_prod(work->scaling->Einv, work->Adelta_x, work->Adelta_x,
                      work->data->m);
        }
...
c_int check_termination(OSQPWorkspace* work, c_int approximate) {
  ...
  if (prim_res_check && dual_res_check) {
    ...
    if (work->settings->scaling && !work->settings->scaled_termination) {
      ...
      vec_ew_prod(work->scaling->E, work->delta_y, work->delta_y,
                  work->data->m);
    }
    ...
    if (work->settings->scaling && !work->settings->scaled_termination) {
      // Update infeasibility certificate
      vec_ew_prod(work->scaling->D, work->delta_x, work->delta_x,
                  work->data->n);
    }
    work->info->obj_val = -OSQP_INFTY;
    exitflag = 1;
