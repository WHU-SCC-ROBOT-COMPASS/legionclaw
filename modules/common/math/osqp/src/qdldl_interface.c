...
static c_int LDL_factor(csc* A, qdldl_solver* p, c_int nvar) {
  ...
  if (sum_Lnz < 0) {
    // Error
#ifdef PRINTING
    c_eprint("Error in KKT matrix LDL factorization when computing the "
             "elimination tree. A is not perfectly upper triangular");
#endif
    return sum_Lnz;
  }
  ...
  // Factor matrix
  factor_status =
      QDLDL_factor(A->n, A->p, A->i, A->x, p->L->p, p->L->i, p->L->x, p->D,
                   p->Dinv, p->Lnz, p->etree, p->bwork, p->iwork, p->fwork);

  if (factor_status < 0) {
    // Error
#ifdef PRINTING
    c_eprint("Error in KKT matrix LDL factorization when computing the nonzero "
...
static c_int permute_KKT(csc** KKT, qdldl_solver* p, c_int Pnz, c_int Anz,
                         c_int m, c_int* PtoKKT, c_int* AtoKKT,
                         c_int* rhotoKKT) {
  c_float* info;
  c_int amd_status;
  c_int* Pinv;
...
#ifdef DLONG
  amd_status = amd_l_order((*KKT)->n, (*KKT)->p, (*KKT)->i, p->P,
                           (c_float*)OSQP_NULL, info);
#else
  amd_status = amd_order((*KKT)->n, (*KKT)->p, (*KKT)->i, p->P,
                         (c_float*)OSQP_NULL, info);
#endif
  if (amd_status < 0)
    return (amd_status);
...
qdldl_solver* init_linsys_solver_qdldl(const csc* P, const csc* A,
  ...
  if (polish) { // Called from polish()
    ...
    KKT_temp = form_KKT(P, A, 0, sigma, p->bp, OSQP_NULL, OSQP_NULL, OSQP_NULL,
                        OSQP_NULL, OSQP_NULL);
    ...
    KKT_temp = form_KKT(P, A, 0, sigma, p->bp, p->PtoKKT, p->AtoKKT,
                        &(p->Pdiag_idx), &(p->Pdiag_n), p->rhotoKKT);

    // Permute matrix
    permute_KKT(&KKT_temp, p, P->p[P->n], A->p[A->n], A->m, p->PtoKKT,
                p->AtoKKT, p->rhotoKKT);
  }
...
c_int update_linsys_solver_matrices_qdldl(qdldl_solver* s, const csc* P,
                                          const csc* A,
                                          const OSQPSettings* settings) {
  c_int kk;
  ...
  return (QDLDL_factor(s->KKT->n, s->KKT->p, s->KKT->i, s->KKT->x, s->L->p,
                       s->L->i, s->L->x, s->D, s->Dinv, s->Lnz, s->etree,
                       s->bwork, s->iwork, s->fwork) < 0);
}
