...
void osqp_start_interrupt_listener(void) { istate = utSetInterruptEnabled(1); }

void osqp_end_interrupt_listener(void) { utSetInterruptEnabled(istate); }

int osqp_is_interrupted(void) { return utIsInterruptPending(); }
...
int osqp_is_interrupted(void) { return int_detected; }
...
static void handle_ctrlc(int dummy) { int_detected = dummy ? dummy : -1; }
