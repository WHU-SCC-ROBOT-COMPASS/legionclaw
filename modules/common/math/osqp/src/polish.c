  ...
  for (j = 0; j < work->data->n; j++) { // Cycle over columns of A
    ...
    for (ptr = work->data->A->p[j]; ptr < work->data->A->p[j + 1]; ptr++) {
      ...
      if (work->pol->A_to_Alow[work->data->A->i[ptr]] != -1) {
        ...
        work->pol->Ared->i[Ared_nnz] =
            work->pol->A_to_Aupp[work->data->A->i[ptr]] + work->pol->n_low;
        work->pol->Ared->x[Ared_nnz++] = work->data->A->x[ptr];
      }
    }
  }
 ...
 */
static void iterative_refinement(OSQPWorkspace* work, LinSysSolver* p,
                                 c_float* z, c_float* b) {
...
c_int polish(OSQPWorkspace* work) {
  ...
  plsh =
      init_linsys_solver(work->data->P, work->pol->Ared, work->settings->delta,
                         OSQP_NULL, work->settings->linsys_solver, 1);
