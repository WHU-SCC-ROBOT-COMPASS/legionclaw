 ...
 * This function is not defined in the header to allow a workaround for the
 * problem that dlsym returns an object instead of a function pointer. However,
 * Windows also needs special care.
 *
 * The method does six attempts to load the symbol. Next to its given name, it
 * also tries variations of lower case and upper case form and with an extra
 * underscore.
...
#ifdef PRINTING
      c_eprint("Cannot find symbol %s in dynamic library, error = %d", symName,
               (int)GetLastError());
#endif
    }
#else
    s = dlsym(h, tripSym);
    err = dlerror(); /* we have only one chance; a successive call to dlerror()
                        returns OSQP_NULL */
    if (err) {
#ifdef PRINTING
      c_eprint("Cannot find symbol %s in dynamic library, error = %s", symName,
               err);
#endif
    } else {
      return s;
