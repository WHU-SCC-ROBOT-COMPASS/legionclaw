...
OSQPWorkspace* osqp_setup(const OSQPData* data, OSQPSettings* settings) {
  ...
  work->linsys_solver =
      init_linsys_solver(work->data->P, work->data->A, work->settings->sigma,
...
c_int osqp_solve(OSQPWorkspace* work) {
    ...
    if (work->settings->adaptive_rho &&
      ...
      if (osqp_toc(work->timer) >
        ...
        if (work->settings->check_termination) {
          ...
          work->settings->adaptive_rho_interval =
              (c_int)c_roundmultiple(iter, work->settings->check_termination);
        } else {
          ...
          // interval.
          work->settings->adaptive_rho_interval =
              (c_int)c_roundmultiple(iter, CHECK_TERMINATION);
        }

        // Make sure the interval is not 0 and at least check_termination times
        work->settings->adaptive_rho_interval =
            c_max(work->settings->adaptive_rho_interval,
  ...
  if (work->first_run) {
    ...
    work->info->run_time = work->info->update_time + work->info->solve_time +
                           work->info->polish_time;
  }
...
c_int osqp_update_bounds(OSQPWorkspace* work, const c_float* l_new,
                         const c_float* u_new) {
  c_int i, exitflag = 0;
 ...
 */
c_int osqp_update_P(OSQPWorkspace* work, const c_float* Px_new,
                    const c_int* Px_new_idx, c_int P_new_n) {
  ...
  exitflag = work->linsys_solver->update_matrices(
      work->linsys_solver, work->data->P, work->data->A, work->settings);
 ...
 *                                 <0: error in update_matrices()
 */
c_int osqp_update_A(OSQPWorkspace* work, const c_float* Ax_new,
                    const c_int* Ax_new_idx, c_int A_new_n) {
  c_int i;        // For indexing
  c_int exitflag; // Exit flag
 ...
 *                                 <0: error in update_matrices()
 */
c_int osqp_update_P_A(OSQPWorkspace* work, const c_float* Px_new,
                      const c_int* Px_new_idx, c_int P_new_n,
                      const c_float* Ax_new, const c_int* Ax_new_idx,
                      c_int A_new_n) {
  c_int i;          // For indexing
  c_int exitflag;   // Exit flag
