...
public:
  ...
  inline void set_obstacle(std::vector<legionclaw::interface::Obstacle> *obstacle) {
    std::lock_guard<std::mutex> lock(*obstacle_mutex_);
    obstacle_.assign(obstacle->begin(), obstacle->end());
  }

  inline void
  set_obstacle(const std::vector<legionclaw::interface::Obstacle> &obstacle) {
    std::lock_guard<std::mutex> lock(*obstacle_mutex_);
    obstacle_ = obstacle;
  }
  ...
  inline void add_obstacle(const legionclaw::interface::Obstacle &obstacle) {
    std::lock_guard<std::mutex> lock(*obstacle_mutex_);
    obstacle_.emplace_back(obstacle);
  }

  inline const legionclaw::interface::Obstacle &obstacle(uint32_t index) const {
    std::lock_guard<std::mutex> lock(*obstacle_mutex_);
    return obstacle_[index];
  }

  inline std::vector<legionclaw::interface::Obstacle> *mutable_obstacle() {
    std::lock_guard<std::mutex> lock(*obstacle_mutex_);
    return &obstacle_;
  }

  inline void
  obstacle(std::vector<legionclaw::interface::Obstacle> &obstacle) const {
    std::lock_guard<std::mutex> lock(*obstacle_mutex_);
    obstacle.assign(obstacle_.begin(), obstacle_.end());
  }

  inline const std::vector<legionclaw::interface::Obstacle> &obstacle() const {
    std::lock_guard<std::mutex> lock(*obstacle_mutex_);
    return obstacle_;
  }
...
protected:
  ...
  //检测出的障碍物数组
  std::vector<legionclaw::interface::Obstacle> obstacle_;
  //错误码（default = OK）
