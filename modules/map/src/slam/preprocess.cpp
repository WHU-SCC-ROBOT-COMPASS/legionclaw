...
void Preprocess::process(const sensor_msgs::msg::PointCloud2::ConstSharedPtr &msg, PointCloudXYZI::Ptr &pcl_out) {
    ...
    for (int i = 0; i < plsize; i++) {
        ...
        p.curvature = pl_orig.points[i].curvature;
...
void Preprocess::pub_func(PointCloudXYZI &pl, double timestamp)
{
...
#if ROS_ENABLE
  output.header.stamp = ros::Time().fromSec(timestamp);
#elif ROS2_ENABLE
  output.header.stamp = rclcpp::Time(timestamp);
#endif
}
#endif
