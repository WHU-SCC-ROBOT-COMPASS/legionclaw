...
// ROOT_DIR was defined in original FAST-LIO-SAM for debug output. Stubbed out for LegionClaw.
#ifndef ROOT_DIR
#define ROOT_DIR "/tmp/"
#endif
...
/**
 * @brief Lightweight Pose6D struct (middleware-agnostic)
 * Replaces the ROS-generated fast_lio_sam::Pose6D
 */
struct Pose6D {
    double offset_time;
    double acc[3];
    ...
    double vel[3];
    double pos[3];
    double rot[9];  // row-major rotation matrix
};

/**
 * @brief Lightweight IMU data struct (middleware-agnostic)
