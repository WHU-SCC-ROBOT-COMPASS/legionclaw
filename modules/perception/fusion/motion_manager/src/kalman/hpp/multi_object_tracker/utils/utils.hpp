...
#include "common/interface/shape.hpp"
...
inline bool isLargeVehicleLabel(const uint8_t label)
{
  ...
  return label == motion_manager::common::DetectedObjectLabel::DO_BUS || label == motion_manager::common::DetectedObjectLabel::DO_TRUCK || label == motion_manager::common::DetectedObjectLabel::DO_TRAILER;
}
...
inline int getNearestCornerOrSurfaceFromObject(
  const motion_manager::interface::DetectedObject & object, const double & yaw,
  const tf2_geometry_msgs::Transform & self_transform)
{
...
inline void calcAnchorPointOffset(
  ...
  const motion_manager::interface::DetectedObject & input_object, const double & yaw,
  ...
  Eigen::Vector2d & tracking_offset)
{
  ...
  offset_object.mutable_kinematics()->mutable_pose_with_covariance()->mutable_pose()->mutable_position()->set_x(
    offset_object.kinematics().pose_with_covariance().pose().position().x() + rotated_offset.x());
  offset_object.mutable_kinematics()->mutable_pose_with_covariance()->mutable_pose()->mutable_position()->set_y(
...
inline void convertConvexHullToBoundingBox(
  const motion_manager::interface::DetectedObject & input_object,
  motion_manager::interface::DetectedObject & output_object)
{
  const Eigen::Vector2d center{
    ...
    orientation.setX(input_object.kinematics().pose_with_covariance().pose().orientation().qx());
    orientation.setY(input_object.kinematics().pose_with_covariance().pose().orientation().qy());
    orientation.setZ(input_object.kinematics().pose_with_covariance().pose().orientation().qz());
    orientation.setW(input_object.kinematics().pose_with_covariance().pose().orientation().qw());

  const auto yaw = tf2::getYaw(orientation);
  ...
  output_object.mutable_kinematics()->mutable_pose_with_covariance()->mutable_pose()->mutable_position()->set_x( 
    output_object.kinematics().pose_with_covariance().pose().position().x() + new_center.x());
  output_object.mutable_kinematics()->mutable_pose_with_covariance()->mutable_pose()->mutable_position()->set_y(
  output_object.kinematics().pose_with_covariance().pose().position().y() + new_center.y());

  output_object.shape().set_type(motion_manager::common::ShapeType::S_BOUNDING_BOX);
  output_object.shape().dimensions().set_x(length);
  output_object.shape().dimensions().set_y(width);
  output_object.shape().dimensions().set_z(height);
}
