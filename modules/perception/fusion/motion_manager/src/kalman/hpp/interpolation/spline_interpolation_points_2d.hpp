...
public:
  ...
  {
    std::vector<tf2_geometry_msgs::Point> points_inner;
    for (const auto & p : points) {
      points_inner.push_back(utils::getPoint(p));
    }
    calcSplineCoefficientsInner(points_inner);
  }

  // TODO(murooka) implement these functions
  // std::vector<tf2_geometry_msgs::Point> getSplineInterpolatedPoints(const double width);
  // std::vector<tf2_geometry_msgs::Pose> getSplineInterpolatedPoses(const double width);

  // pose (= getSplineInterpolatedPoint + getSplineInterpolatedYaw)
  tf2_geometry_msgs::Pose getSplineInterpolatedPose(const size_t idx, const double s) const;

  // point
  tf2_geometry_msgs::Point getSplineInterpolatedPoint(const size_t idx, const double s) const;
...
private:
  void calcSplineCoefficientsInner(const std::vector<tf2_geometry_msgs::Point> & points);
