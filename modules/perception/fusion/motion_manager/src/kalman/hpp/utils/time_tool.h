...
#include "common/interface/time.hpp"
...
public:
  ...
  static motion_manager::interface::Time Now2TmeStruct() {
    motion_manager::interface::Time time;
    time.set_sec(Now2Seconds());
    time.set_nsec((uint32_t)(int64_t)(Now2Ns() - time.sec() * 1000000000));
    return time;
  }

  static int64_t TimeStruct2Ms(motion_manager::interface::Time time) {
    int64_t ms;
    ms = time.sec();
    ms *= 1000;
  ...
  static int64_t TimeStruct2Us(motion_manager::interface::Time time) {
    int64_t us;
    us = time.sec();
    us *= 1000000;
    us += time.nsec() / 1000;
  ...
  static int64_t TimeStruct2Ns(motion_manager::interface::Time time) {
    int64_t us;
    us = time.sec();
    us *= 1000000000;
    us += time.nsec();
  ...
  static double TimeStruct2S(motion_manager::interface::Time time) {
    double s;
    s = (double)time.sec();
  ...
  static double GetTimeDiffNow(motion_manager::interface::Time time) {
    return NowToSeconds() - TimeStruct2S(time);
  }

  static double GetTimeDiffNow(double time) { return NowToSeconds() - time; }

  static double GetTimeDiff(motion_manager::interface::Time s,
                            motion_manager::interface::Time e) {
    return (TimeStruct2S(e) - TimeStruct2S(s));
  }

  static double GetTimeDiff(double s, double e) { return (e - s); }

  static motion_manager::interface::Time Second2Time(double s) {
    motion_manager::interface::Time t;
    int int_sec = (int)s;
