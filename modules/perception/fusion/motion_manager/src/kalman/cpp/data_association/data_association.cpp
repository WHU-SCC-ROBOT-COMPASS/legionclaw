...
double getMahalanobisDistance(
  const tf2_geometry_msgs::Point & measurement, const tf2_geometry_msgs::Point & tracker,
...
Eigen::Matrix2d getXYCovariance(const tf2_geometry_msgs::PoseWithCovariance & pose_covariance)
{
...
double getFormedYawAngle(
  const tf2_geometry_msgs::Quaternion & measurement_quat,
  const tf2_geometry_msgs::Quaternion & tracker_quat, const bool distinguish_front_or_back = true)
{
  const double measurement_yaw =
    utils::normalizeRadian(tf2::getYaw(measurement_quat));
  const double tracker_yaw = utils::normalizeRadian(tf2::getYaw(tracker_quat));
...
Eigen::MatrixXd DataAssociation::calcScoreMatrix(
  const motion_manager::interface::DetectedObjects & measurements,
  const std::list<std::shared_ptr<Tracker>> & trackers)
{
  ...
  for (auto tracker_itr = trackers.begin(); tracker_itr != trackers.end();
    ...
    for (size_t measurement_idx = 0; measurement_idx < measurements.objects().size();
         ++measurement_idx) {
      const motion_manager::interface::DetectedObject & measurement_object =
      ...
      if (can_assign_matrix_(tracker_label, measurement_label)) {
        motion_manager::interface::TrackedObject tracked_object;
        double time_sec = TimeTool::TimeStruct2S(measurements.header().stamp());
        motion_manager::interface::Time bazyze_time = motion_manager::common::TimeTool::Second2Time(time_sec);
        (*tracker_itr)->getTrackedObject(bazyze_time, tracked_object);
        // tracked_object.object_id().uuid().resize(16);
        ...
        measurement_object_point.x = measurement_object.kinematics().pose_with_covariance().pose().position().x();
        measurement_object_point.y = measurement_object.kinematics().pose_with_covariance().pose().position().y();
        measurement_object_point.z = measurement_object.kinematics().pose_with_covariance().pose().position().z();
        tracked_object_point.x = tracked_object.kinematics().pose_with_covariance().pose().position().x();
        tracked_object_point.y = tracked_object.kinematics().pose_with_covariance().pose().position().y();
        tracked_object_point.z = tracked_object.kinematics().pose_with_covariance().pose().position().z();
        const double dist = utils::calcDistance2d(measurement_object_point,tracked_object_point);
        // const double dist = utils::calcDistance2d(
        ...
        if (passed_gate) {
          ...
          // TODO dcw : motion_manager::interface::Quaternion to tf2_geometry_msgs::Quaternion     done

          tf2_geometry_msgs::Quaternion mea_qua_tmp;
          tf2_geometry_msgs::Quaternion tra_qua_tmp;
          mea_qua_tmp.x = measurement_object.kinematics().pose_with_covariance().pose().orientation().qx();
          mea_qua_tmp.y = measurement_object.kinematics().pose_with_covariance().pose().orientation().qy();
          mea_qua_tmp.z = measurement_object.kinematics().pose_with_covariance().pose().orientation().qz();
          mea_qua_tmp.w = measurement_object.kinematics().pose_with_covariance().pose().orientation().qw();

          tra_qua_tmp.x = tracked_object.kinematics().pose_with_covariance().pose().orientation().qx();
          tra_qua_tmp.y = tracked_object.kinematics().pose_with_covariance().pose().orientation().qy();
          tra_qua_tmp.z = tracked_object.kinematics().pose_with_covariance().pose().orientation().qz();
          tra_qua_tmp.w = tracked_object.kinematics().pose_with_covariance().pose().orientation().qw();
