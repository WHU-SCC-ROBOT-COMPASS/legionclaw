...
std::array<std::vector<double>, 4> getBaseValues(
  const std::vector<tf2_geometry_msgs::Point> & points)
{
  // calculate x, y
...
tf2_geometry_msgs::Pose SplineInterpolationPoints2d::getSplineInterpolatedPose(
  const size_t idx, const double s) const
{
  tf2_geometry_msgs::Pose pose;
  pose.position = getSplineInterpolatedPoint(idx, s);
  tf2::Quaternion quaternion=  tf2::createQuaternionFromYaw(getSplineInterpolatedYaw(idx, s));
...
tf2_geometry_msgs::Point SplineInterpolationPoints2d::getSplineInterpolatedPoint(
  const size_t idx, const double s) const
{
  ...
  tf2_geometry_msgs::Point geom_point;
  geom_point.x = x;
  geom_point.y = y;
  geom_point.z = z;
  return geom_point;
