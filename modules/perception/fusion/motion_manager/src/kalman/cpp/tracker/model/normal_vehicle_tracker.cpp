  ...
  tracking_offset_(Eigen::Vector2d::Zero())
{
  ...
  /*
    r_stddev_x, r_stddev_y, r_stddev_yaw, r_stddev_vel 定义了在车辆坐标系中的测量噪声标准差。
    这些值分别表示位置（x, y），偏航角和速度的噪声标准差。ekf_params_ 是扩展卡尔曼滤波器（EKF）的参数对象，存储这些噪声的协方差矩阵。
  */
  // measurement noise covariance: detector uncertainty + ego vehicle motion uncertainty    测量噪声协方差
  float r_stddev_x = 0.5;                                  // in vehicle coordinate [m]     x方向的标准差
  float r_stddev_y = 0.4;                                  // in vehicle coordinate [m]     y方向的标准差
  float r_stddev_yaw = utils::deg2rad(20);  // in map coordinate [rad]                偏航角的标准差
  float r_stddev_vel = 1.0;                                // in object coordinate [m/s]    速度的标准差
  ...
  // initial state covariance   初始化状态协方差
  /*
    p0_stddev_x, p0_stddev_y, p0_stddev_yaw, p0_stddev_vel, p0_stddev_slip 定义了状态向量的初始协方差标准差。每个标准差都对应一个状态变量，如位置、偏航角、速度和滑移角。
  */
  float p0_stddev_x = 1.0;                                     // in object coordinate [m]  位置x的标准差
  float p0_stddev_y = 0.3;                                     // in object coordinate [m]  位置y的标准差
  float p0_stddev_yaw = utils::deg2rad(25);     // in map coordinate [rad]            偏航角的标准差
  float p0_stddev_vel = utils::kmph2mps(1000);  // in object coordinate [m/s]         速度的标准差
  float p0_stddev_slip = utils::deg2rad(5);     // in object coordinate [rad/s]       滑移角的标准差
  ...
  ekf_params_.q_stddev_acc_long = 9.8 * 0.35;  // [m/(s*s)] uncertain longitudinal acceleration     纵向加速度噪声
  ekf_params_.q_stddev_acc_lat = 9.8 * 0.15;   // [m/(s*s)] uncertain lateral acceleration          横向加速度噪声
  ekf_params_.q_stddev_yaw_rate_min = 
    utils::deg2rad(1.5);  // [rad/s] uncertain yaw change rate                                最小偏航速率噪声
  ekf_params_.q_stddev_yaw_rate_max =
    utils::deg2rad(15.0);  // [rad/s] uncertain yaw change rate                               最大偏航速率噪声
  float q_stddev_slip_rate_min =
    utils::deg2rad(0.3);  // [rad/s] uncertain slip angle change rate                         滑移角速率最小噪声
  float q_stddev_slip_rate_max =
    utils::deg2rad(10.0);  // [rad/s] uncertain slip angle change rate                        滑移角速率最大噪声
  ...
  ekf_params_.q_max_slip_angle = utils::deg2rad(30);  // [rad] max slip angle                 最大滑移角
  // limitations
  max_vel_ = utils::kmph2mps(100);                      // [m/s]
  max_slip_ = utils::deg2rad(30);                       // [rad]
  velocity_deviation_threshold_ = utils::kmph2mps(10);  // [m/s]
  ...
  if (object.shape().type() == motion_manager::common::ShapeType::S_BOUNDING_BOX) {
    ...
    motion_manager::interface::DetectedObject bbox_object;
...
bool NormalVehicleTracker::predict(const motion_manager::interface::Time & time)
{
  ...
  const motion_manager::interface::DetectedObject & object)
{
  ...
  motion_manager::interface::DetectedObject offset_object;
  ...
  const motion_manager::interface::Time & time, motion_manager::interface::TrackedObject & object) const
{
  ...
  // position
  if (fabs(object_.kinematics().pose_with_covariance_vehicle().pose().position().x())<20||fabs(object_.kinematics().pose_with_covariance_vehicle().pose().position().y())<20)
  {
      pose_with_cov.mutable_pose()->mutable_position()->set_x(object_.kinematics().pose_with_covariance().pose().position().x());
      pose_with_cov.mutable_pose()->mutable_position()->set_y(object_.kinematics().pose_with_covariance().pose().position().y());
  ...
  // twist
  twist_with_cov.mutable_twist()->mutable_linear()->set_x(X_t(IDX::VEL) * std::cos(X_t(IDX::YAW)));
  twist_with_cov.mutable_twist()->mutable_linear()->set_y(X_t(IDX::VEL) * std::sin(X_t(IDX::YAW)));
  twist_with_cov.mutable_twist()->mutable_angular()->set_z(X_t(IDX::VEL) * std::sin(X_t(IDX::SLIP)) / lr_);
  /* twist covariance
  ...
  twist_with_cov.set_covariance(utils::MSG_COV_IDX::X_X, twist_cov_mat(0, 0));
  twist_with_cov.set_covariance(utils::MSG_COV_IDX::X_Y, twist_cov_mat(0, 1));
  twist_with_cov.set_covariance(utils::MSG_COV_IDX::X_YAW, twist_cov_mat(0, 2));
  twist_with_cov.set_covariance(utils::MSG_COV_IDX::Y_X, twist_cov_mat(1, 0));
  twist_with_cov.set_covariance(utils::MSG_COV_IDX::Y_Y, twist_cov_mat(1, 1));
  twist_with_cov.set_covariance(utils::MSG_COV_IDX::Y_YAW, twist_cov_mat(1, 2));
  twist_with_cov.set_covariance(utils::MSG_COV_IDX::YAW_X, twist_cov_mat(2, 0));
  twist_with_cov.set_covariance(utils::MSG_COV_IDX::YAW_Y, twist_cov_mat(2, 1));
  twist_with_cov.set_covariance(utils::MSG_COV_IDX::YAW_YAW, twist_cov_mat(2, 2));
...
const auto ekf_pose_yaw = tf2::getYaw(orientation);
  // TODO dcw : Polygon2d to motion_manager::interface::Ploygon
  ...
  const motion_manager::interface::DetectedObject & object)
{
    ...
    orientation.setZ(object.kinematics().pose_with_covariance().pose().orientation().qz());
    orientation.setW(object.kinematics().pose_with_covariance().pose().orientation().qw());
  double measurement_yaw = utils::normalizeRadian(
