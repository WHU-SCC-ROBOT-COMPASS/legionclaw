  ...
  z_(object.kinematics().pose_with_covariance().pose().position().z())
{
  ...
  float q_stddev_vx = utils::kmph2mps(20);   // [m/(s*s)]
  float q_stddev_vy = utils::kmph2mps(20);   // [m/(s*s)]
  float r_stddev_x = 1.0;                                   // [m]
  float r_stddev_y = 1.0;                                   // [m]
  float p0_stddev_x = 1.0;                                  // [m/s]
  float p0_stddev_y = 1.0;                                  // [m/s]
  float p0_stddev_vx = utils::kmph2mps(10);  // [m/(s*s)]
  float p0_stddev_vy = utils::kmph2mps(10);  // [m/(s*s)]
  ...
  max_vx_ = utils::kmph2mps(60);  // [m/s]
  max_vy_ = utils::kmph2mps(60);  // [m/s]
...
bool UnknownTracker::predict(const motion_manager::interface::Time & time)
{
...
bool UnknownTracker::predict(const double dt, KalmanFilter & ekf) const
{
  ...
  if (!ekf.predict(X_next_t, A, Q)) {
    ...
    AWARN <<"Pedestrian : Cannot predict";
  }
  ...
  const tf2_geometry_msgs::Transform & /*self_transform*/)
{
  ...
  if (0.01 /*10msec*/ < std::fabs(TimeTool::TimeStruct2S(time)-TimeTool::TimeStruct2S(last_update_time_))) {
    ...
    AWARN << "Pedestrian : There is a large gap between predicted time and measurement time. (%f)",
      TimeTool::TimeStruct2S(time)-TimeTool::TimeStruct2S(last_update_time_);
  }
  ...
  const motion_manager::interface::Time & time, motion_manager::interface::TrackedObject & object) const
{
  ...
  double yaw_cov = 0.1 * 0.1;  // TODO(yukkysaito) Currently tentative
  ...
  pose_with_cov.set_covariance(utils::MSG_COV_IDX::YAW_YAW, yaw_cov);
  ...
  const auto pose_yaw = tf2::getYaw(orientation);
  const double cos = std::cos(-pose_yaw);
  const double sin = std::sin(-pose_yaw);
  twist_with_cov.mutable_twist()->mutable_linear()->set_x(cos * X_t(IDX::VX) - sin * X_t(IDX::VY));
  twist_with_cov.mutable_twist()->mutable_linear()->set_y(sin * X_t(IDX::VX) + cos * X_t(IDX::VY));
  ...
  double wz_cov = 0.1 * 0.1;  // TODO(yukkysaito) Currently tentative
  twist_with_cov.covariance().resize(36);
  twist_with_cov.set_covariance(utils::MSG_COV_IDX::X_X,P(IDX::VX, IDX::VX));
  twist_with_cov.set_covariance(utils::MSG_COV_IDX::Y_Y,P(IDX::VY, IDX::VY));
  ...
  twist_with_cov.set_covariance(utils::MSG_COV_IDX::YAW_YAW,wz_cov);
