  ...
  z_(object.kinematics().pose_with_covariance().pose().position().z())
{
  ...
  max_slip_ = utils::deg2rad(30);   // [rad/s]
...
bool BicycleTracker::predict(const motion_manager::interface::Time & time)
{
  ...
  const motion_manager::interface::DetectedObject & object)
{
  ...
  if (
    ...
    utils::ORIENTATION_AVAILABILITY::SIGN_UNKNOWN) {  // has 180 degree
                                                                                   // uncertainty
    // fix orientation
    ...
    object.kinematics().orientation_availability() ==
    utils::ORIENTATION_AVAILABILITY::AVAILABLE) {  // know full angle
  ...
  const tf2_geometry_msgs::Transform & /*self_transform*/)
{
  ...
  if (0.01 /*10msec*/ < std::fabs(TimeTool::TimeStruct2S(time)-TimeTool::TimeStruct2S(last_update_time_))) {
    ...
    // ROS_WARN("There is a large gap between predicted time and measurement time. (%f)",
    //   TimeTool::TimeStruct2S(time)-TimeTool::TimeStruct2S(last_update_time_));
