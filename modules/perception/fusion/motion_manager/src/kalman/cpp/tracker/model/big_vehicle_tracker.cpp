  ...
  tracking_offset_(Eigen::Vector2d::Zero())
{
  ...
  float r_stddev_yaw = utils::deg2rad(20);  // [rad]
  ...
  float p0_stddev_yaw = utils::deg2rad(25);     // in map coordinate [rad]
  float p0_stddev_vel = utils::kmph2mps(1000);  // in object coordinate [m/s]
  float p0_stddev_slip = utils::deg2rad(5);     // in object coordinate [rad/s]
  ...
  ekf_params_.q_stddev_yaw_rate_min =
    utils::deg2rad(1.5);  // [rad/s] uncertain yaw change rate
  ekf_params_.q_stddev_yaw_rate_max =
    utils::deg2rad(15.0);  // [rad/s] uncertain yaw change rate
  float q_stddev_slip_rate_min =
    utils::deg2rad(0.3);  // [rad/s] uncertain slip angle change rate
  float q_stddev_slip_rate_max =
    utils::deg2rad(10.0);  // [rad/s] uncertain slip angle change rate
  ...
  ekf_params_.q_max_slip_angle = utils::deg2rad(30);  // [rad] max slip angle
  // limitations
...
bool BigVehicleTracker::predict(const motion_manager::interface::Time & time)
{
  ...
  const motion_manager::interface::Time & time, motion_manager::interface::TrackedObject & object) const
{
  ...
  twist_with_cov.mutable_twist()->mutable_angular()->set_z(X_t(IDX::VEL) * std::sin(X_t(IDX::SLIP)) / lr_);  // yaw_rate = vel_k * sin(slip_k) / l_r
