...
//   wqos.resource_limits().allocated_samples = 100;
  ...
  // ObstacleList
  out_obstacle_list_type_.reset(new eprosima::fastdds::dds::TypeSupport(
      new ros2_interface::msg::ObstacleListPubSubType()));
  out_obstacle_list_type_->register_type(participant_);
  publisher_ = participant_->create_publisher(PUBLISHER_QOS_DEFAULT, nullptr);
  topic_ = participant_->create_topic(
      "rt/perception/fusion/motion_manager/MMObstacleList",
      out_obstacle_list_type_->get_type_name(), TOPIC_QOS_DEFAULT);
  out_obstacle_list_writer_ =
  ...
  topic_ = participant_->create_topic(
      ...
      location_type_->get_type_name(), TOPIC_QOS_DEFAULT);
  location_reader_ = subscriber_->create_datareader(topic_, rqos, this);

  in_obstacle_list_type_.reset(new eprosima::fastdds::dds::TypeSupport(
      new ros2_interface::msg::ObstacleListPubSubType()));
  in_obstacle_list_type_->register_type(participant_);
  subscriber_ =
      participant_->create_subscriber(SUBSCRIBER_QOS_DEFAULT, nullptr);
  topic_ = participant_->create_topic(
      "rt/perception/camera/yolov7_detect/ObstacleList",
      in_obstacle_list_type_->get_type_name(), TOPIC_QOS_DEFAULT);
  in_obstacle_list_reader_ = subscriber_->create_datareader(topic_, rqos, this);
...
template <typename T>
void DdsMessageManager<T>::PublishObstacleListOutput(
  ...
  out_obstacle_list_writer_->write(&obstacle_list);
}
...
template <typename T>
void DdsMessageManager<T>::HandleINObstacleListMessage(
...
void DdsMessageManager<T>::on_data_available(
  ...
  if (!reader->get_topicdescription()->get_name().compare(
    ...
    eprosima::fastdds::dds::SampleInfo info;
    if (in_obstacle_list_reader_->take_next_sample(&msg, &info) ==
        ReturnCode_t::RETCODE_OK) {
      HandleINObstacleListMessage(&msg);
    }
  }
}
template <typename T> DdsMessageManager<T>::~DdsMessageManager() {
  if (out_obstacle_list_writer_ != nullptr) {
    publisher_->delete_datawriter(out_obstacle_list_writer_);
  }
  ...
  if (location_reader_ != nullptr) {
    subscriber_->delete_datareader(location_reader_);
  }
  if (in_obstacle_list_reader_ != nullptr) {
    subscriber_->delete_datareader(in_obstacle_list_reader_);
  }
  if (topic_ != nullptr) {
    participant_->delete_topic(topic_);
  }
