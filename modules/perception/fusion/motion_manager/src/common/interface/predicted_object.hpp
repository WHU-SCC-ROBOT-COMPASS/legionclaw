...
#include "predicted_object_kinematics.hpp"
...
public:
    ...
    inline void set_object_id(const motion_manager::interface::UUID& object_id)
    {
        object_id_ = object_id;
        object_id_ptr_ = &object_id_;
    }

    inline const motion_manager::interface::UUID& object_id() const
    {
        return object_id_;
    }

    inline motion_manager::interface::UUID* mutable_object_id()
    {
        return& object_id_;
    }

    inline bool has_object_id()
    {
        return (object_id_ptr_ != nullptr);
    }
    ...
    inline void set_kinematics(const motion_manager::interface::PredictedObjectKinematics& kinematics)
    {
        kinematics_ = kinematics;
        kinematics_ptr_ = &kinematics_;
    }

    inline const motion_manager::interface::PredictedObjectKinematics& kinematics() const
    {
        return kinematics_;
    }

    inline motion_manager::interface::PredictedObjectKinematics* mutable_kinematics()
    {
        return& kinematics_;
    }
...
void operator = (const PredictedObject& predicted_object){
    CopyFrom(predicted_object);
  }

  void CopyFrom(const PredictedObject& predicted_object ){
    object_id_ = predicted_object.object_id();
    existence_probability_ = predicted_object.existence_probability();
    classification_ = predicted_object.classification();
    kinematics_ = predicted_object.kinematics();
    shape_ = predicted_object.shape();
  }

protected:
    std::shared_ptr<std::mutex> classification_mutex_;
    motion_manager::interface::UUID object_id_;
    motion_manager::interface::UUID* object_id_ptr_ = nullptr;
    ...
    motion_manager::interface::PredictedObjectKinematics kinematics_;
    motion_manager::interface::PredictedObjectKinematics* kinematics_ptr_ = nullptr;
