/**
 * @file    float_32multi_array_stamped.hpp
...
public:
    ...
    inline void set_data(std::vector<float>* data)
    {
        std::lock_guard<std::mutex> lock(*data_mutex_);
        data_.assign(data->begin(), data->end());
    }

    inline void set_data(const std::vector<float>& data)
    {
        std::lock_guard<std::mutex> lock(*data_mutex_);
        data_=data;
    }

    inline void set_data(const uint32_t index,float& data)
    {
        std::lock_guard<std::mutex> lock(*data_mutex_);
        data_[index] = data;
    }
    ...
    inline const float& data(uint32_t index) const
    {
        std::lock_guard<std::mutex> lock(*data_mutex_);
        return data_[index];
    }

    inline std::vector<float>* mutable_data()
    {
        std::lock_guard<std::mutex> lock(*data_mutex_);
        return& data_;
    }

    inline void data(std::vector<float>& data) const
    {
        std::lock_guard<std::mutex> lock(*data_mutex_);
...
void operator = (const Float32MultiArrayStamped& float_32multi_array_stamped){
    CopyFrom(float_32multi_array_stamped);
  }

  void CopyFrom(const Float32MultiArrayStamped& float_32multi_array_stamped ){
    stamp_ = float_32multi_array_stamped.stamp();
    layout_ = float_32multi_array_stamped.layout();
    data_ = float_32multi_array_stamped.data();
  }
