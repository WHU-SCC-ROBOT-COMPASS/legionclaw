/**
 * @file    int_32multi_array_stamped.hpp
...
public:
    ...
    ~Int32MultiArrayStamped() = default;
    ...
    inline void set_data(std::vector<int32_t>* data)
    {
        std::lock_guard<std::mutex> lock(*data_mutex_);
        data_.assign(data->begin(), data->end());
    }

    inline void set_data(const std::vector<int32_t>& data)
    {
        std::lock_guard<std::mutex> lock(*data_mutex_);
        data_=data;
    }

    inline void set_data(const uint32_t index,int32_t& data)
    {
        std::lock_guard<std::mutex> lock(*data_mutex_);
        data_[index] = data;
    }
    ...
    inline const int32_t& data(uint32_t index) const
    {
        std::lock_guard<std::mutex> lock(*data_mutex_);
        return data_[index];
    }

    inline std::vector<int32_t>* mutable_data()
    {
        std::lock_guard<std::mutex> lock(*data_mutex_);
        return& data_;
    }

    inline void data(std::vector<int32_t>& data) const
    {
        std::lock_guard<std::mutex> lock(*data_mutex_);
        data.assign(data_.begin(),data_.end());
    }

    inline const std::vector<int32_t>& data() const
    {
        std::lock_guard<std::mutex> lock(*data_mutex_);
        return data_;
    }
...
void operator = (const Int32MultiArrayStamped& int_32multi_array_stamped){
    CopyFrom(int_32multi_array_stamped);
  }

  void CopyFrom(const Int32MultiArrayStamped& int_32multi_array_stamped ){
    stamp_ = int_32multi_array_stamped.stamp();
    layout_ = int_32multi_array_stamped.layout();
    data_ = int_32multi_array_stamped.data();
  }
