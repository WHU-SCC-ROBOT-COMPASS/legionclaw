...
inline Eigen::MatrixXf PointCloud2ToEigenMat(const PointCloud2::ConstSharedPtr &msg) {
  ...
  if (has_intensity) {
    // 如果有intensity字段，提取4列（x, y, z, intensity）
    sensor_msgs::PointCloud2ConstIterator<float> msg_intensity(*msg, intensity_field_name);
    points.resize(num_points, 4);
    for (size_t i = 0; i < num_points; ++i, ++msg_x, ++msg_y, ++msg_z, ++msg_intensity) {
      points.row(i) << *msg_x, *msg_y, *msg_z, *msg_intensity;
    }
  } else {
    // 如果没有intensity字段，仍然创建4列矩阵，intensity设为0
    // 注意：这会导致RNR功能无法正常工作，但不会导致程序崩溃
    RCLCPP_WARN_ONCE(rclcpp::get_logger("patchworkpp_node"),
                     "PointCloud2 message does not contain intensity field. "
                     "RNR (Reflected Noise Removal) will be disabled.");
    points.resize(num_points, 4);
    for (size_t i = 0; i < num_points; ++i, ++msg_x, ++msg_y, ++msg_z) {
      points.row(i) << *msg_x, *msg_y, *msg_z, 0.0f;
    }
  }
