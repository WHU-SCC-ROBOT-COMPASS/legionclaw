...
template <typename T> void DdsMessageManager<T>::Init(T* t) {
  ...
  // ObstacleList
  lcd_obstacle_list_type_.reset(new eprosima::fastdds::dds::TypeSupport(
      new ros2_interface::msg::ObstacleListPubSubType()));
  lcd_obstacle_list_type_->register_type(participant_);
  publisher_ = participant_->create_publisher(PUBLISHER_QOS_DEFAULT, nullptr);
  lcd_obstacle_list_topic_ = participant_->create_topic(
      "rt/perception/lidar/lidar_cluster_detect/LCDObstacleList",
      lcd_obstacle_list_type_->get_type_name(), TOPIC_QOS_DEFAULT);
  lcd_obstacle_list_writer_ =
      publisher_->create_datawriter(lcd_obstacle_list_topic_, wqos, nullptr);
  ...
  faults_topic_ = participant_->create_topic(
      "rt/perception/lidar/lidar_cluster_detect/Faults",
  ...
  no_ground_points_topic_ = participant_->create_topic(
      "rt/NoGroundPoints", no_ground_points_type_->get_type_name(),
      TOPIC_QOS_DEFAULT);
...
template <typename T> void DdsMessageManager<T>::TaskStart() {
  ...
  rqos.reliability().kind = RELIABLE_RELIABILITY_QOS;
  rqos.endpoint().history_memory_policy = DYNAMIC_REUSABLE_MEMORY_MODE;
  no_ground_points_reader_ =
      subscriber_->create_datareader(no_ground_points_topic_, rqos, this);
...
void DdsMessageManager<T>::PublishObstacleList(
  ...
  lcd_obstacle_list_writer_->write(&obstacle_list);
}
...
template <typename T>
void DdsMessageManager<T>::HandleNoGroundPointsMessage(
...
void DdsMessageManager<T>::on_data_available(
  ...
  if (!reader->get_topicdescription()->get_name().compare(
    ...
    eprosima::fastdds::dds::SampleInfo info;
    if (no_ground_points_reader_->take_next_sample(&msg, &info) ==
        ReturnCode_t::RETCODE_OK) {
      HandleNoGroundPointsMessage(&msg);
    }
  }
  return;
}
