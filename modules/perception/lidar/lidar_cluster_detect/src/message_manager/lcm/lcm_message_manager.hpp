...
template <typename T> bool LcmMessageManager<T>::Activate() {
  ...
  point_cloud_sub_ = lcm_->subscribe(
      "NoGroundPoints", &LcmMessageManager::HandlePointCloudMessage, this);
...
template <typename T> bool LcmMessageManager<T>::DeActivate() {
  ...
  std::lock_guard<std::mutex> lock(mutex_);
  lcm_->unsubscribe(point_cloud_sub_);
...
void LcmMessageManager<T>::PublishObstacleList(
  ...
  lcm_->publish("/perception/lidar/lidar_cluster_detect/LCDObstacleList",
                &obstacle_list);
}
...
void LcmMessageManager<T>::PublishFaults(legionclaw::interface::Faults msg) {
  ...
  lcm_->publish("/perception/lidar/lidar_cluster_detect/Faults", &faults);
}
