...
template <typename T> void RosMessageManager<T>::Init(T* t) {
  ...
  obstacle_list_pub_ = nh_.advertise<::ros_interface::ObstacleList>(
      "/perception/lidar/lidar_cluster_detect/LCDObstacleList", 10);

  faults_pub_ = nh_.advertise<::ros_interface::Faults>(
      "/perception/lidar/lidar_cluster_detect/Faults", 10);
...
template <typename T> void RosMessageManager<T>::TaskStart() {
  ...
  point_cloud_sub_ = nh_.subscribe(
      "NoGroundPoints", 30, &RosMessageManager::HandlePointCloudMessage, this);
