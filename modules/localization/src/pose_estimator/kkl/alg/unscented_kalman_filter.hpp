 ...
 * @author koide
 * 16/02/01
 * ROS2 version - no ROS dependencies (pure Eigen + STL)
 **/
...
public:
  ...
  /**
   * @brief predict with control input
  ...
  /**
   * @brief correct with measurement
  ...
  void correct(const VectorXt& measurement) {
    ...
    const auto& K_mat = kalman_gain;

    VectorXt ext_mean = ext_mean_pred + K_mat * (measurement - expected_measurement_mean);
    MatrixXt ext_cov = ext_cov_pred - K_mat * expected_measurement_cov * K_mat.transpose();
...
private:
  ...
  /**
   * @brief make covariance matrix positive finite (disabled by default)
   */
  void ensurePositiveFinite(MatrixXt& /*cov*/) {
    // Disabled by default for performance
    // To enable, uncomment the body
    return;
  }
