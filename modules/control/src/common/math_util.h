...
static inline double sgn(double v) { return (v >= 0) ? 1 : -1; }
...
static inline double streeing_to_ks(double steering_angle, int32_t k_trans,
                                    double vehicle_length) {
  double k_s = 0;

  k_s = tan(to_radians(steering_angle / k_trans)) / vehicle_length;
