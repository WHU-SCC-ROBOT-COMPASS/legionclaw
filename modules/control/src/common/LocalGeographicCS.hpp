...
struct LocalGeographicCS {
  ...
  void ll2xy(const ItIn& lat_begin, const ItIn& lat_end, const ItIn& lon_begin,
             const ItOut& x_begin, const ItOut& y_begin) const;
...
inline void LocalGeographicCS::ll2xy(double lat, double lon, double& x,
                                     double& y) const {
...
inline std::pair<double, double> LocalGeographicCS::ll2xy(double lat,
                                                          double lon) const {
  double x, y;
  ll2xy(lat, lon, x, y);
...
inline void LocalGeographicCS::xy2ll(double x, double y, double& lat,
                                     double& lon) const {
  x += _x0;
  y += _y0;
...
inline std::pair<double, double> LocalGeographicCS::xy2ll(double x,
                                                          double y) const {
  double lat, lon;
  xy2ll(x, y, lat, lon);
...
// operate on containers
template <class ItIn, class ItOut>
void LocalGeographicCS::ll2xy(const ItIn& lat_begin, const ItIn& lat_end,
...
template <class ItIn, class ItOut>
void LocalGeographicCS::xy2ll(const ItIn& x_begin, const ItIn& x_end,
  ...
  for (; x != x_end; lat++, lon++, x++, y++) xy2ll(*x, *y, *lat, *lon);
}
