...
# Loop for the subscriber
    ...
    # Create a publisher on the topic
    try:
        pobstacle_list_sub = fastdds_obstacle_list.ObstacleListDataReader(0, "rt/perception/fusion/preprocessor/PObstacleList")
    ...
    pobstacle_list = fastdds_obstacle_list.ObstacleList()
    bev_tracking_obstacle_list = fastdds_obstacle_list.ObstacleList()

    try:
        while True:
            # Wait for a message for 2 seconds
            if (pobstacle_list_sub.wait_for_sample(2)):
                # Read the received message
                if (pobstacle_list_sub.take_sample(pobstacle_list)):
                    print("success")
